...
export const waitForXDomainTransaction = async (
  ...
  const { src, dest } =
    direction === Direction.L1ToL2 ? { src: watcher.l1, dest: watcher.l2 } : { src: watcher.l2, dest: watcher.l1 }
  ...
  const remoteReceipt = (await watcher.getTransactionReceipt(dest, xDomainMsgHash)) as TransactionReceipt
